...
const savePage = () => {
  ...
  axios
    .patch("/blogs/" + route.params.id, blogData.value, {
      headers: {
    ...
    .catch((err) => {
      ...
      window.alert(
        "Die Blogbeitrag konnte nicht gespeichert werden. Bitte versuche es erneut."
      );
    });
};
...
onMounted(async () => {
  if (route.params.id) {
    ...
    blogData.value["name:de"] = res.data.translations[0].name;
